...
public record UpdateParallelCorpus(
    ...
    WordAlignmentParallelCorpusUpdateConfigDto CorpusConfig
) : IRequest<UpdateParallelCorpusResponse>;

public record UpdateParallelCorpusResponse(WordAlignmentParallelCorpusDto Corpus);
        ...
        return dataAccessContext.WithTransactionAsync(
            async (ct) =>
            {
                ...
                await wordAlignments.DeleteAllAsync(
                    wa => wa.CorpusRef == request.ParallelCorpusId,
                    cancellationToken: ct
                );
...
public partial class WordAlignmentEnginesController
{
    ...
    /// </summary>
    /// <remarks>
    /// Will completely replace the parallel corpus' file associations. Will not affect jobs already queued or running. Will not affect existing word alignments until new build is complete.
    /// </remarks>

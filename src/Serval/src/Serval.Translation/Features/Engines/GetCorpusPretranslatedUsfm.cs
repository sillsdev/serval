...
public record GetCorpusPretranslatedUsfm(
    string Owner,
    string EngineId,
    string CorpusId,
    string TextId,
...
) : IRequest<GetCorpusPretranslatedUsfmResponse>;

public record GetCorpusPretranslatedUsfmResponse(PretranslationStatus Status, string? Usfm = null);

public class GetCorpusPretranslatedUsfmHandler(
    IRepository<Engine> engines,
    IUsfmGenerationService pretranslationService,
    ILogger<GetCorpusPretranslatedUsfmHandler> logger
) : IRequestHandler<GetCorpusPretranslatedUsfm, GetCorpusPretranslatedUsfmResponse>
{
    public async Task<GetCorpusPretranslatedUsfmResponse> HandleAsync(
        GetCorpusPretranslatedUsfm request,
...
public partial class TranslationEnginesController
{
    ...
    public async Task<IActionResult> GetCorpusPretranslatedUsfmAsync(
        ...
        [FromServices] IRequestHandler<GetCorpusPretranslatedUsfm, GetCorpusPretranslatedUsfmResponse> handler,
        CancellationToken cancellationToken
    )
    {
        GetCorpusPretranslatedUsfmResponse response = await handler.HandleAsync(
            new(
                Owner,
                id,
                corpusId,

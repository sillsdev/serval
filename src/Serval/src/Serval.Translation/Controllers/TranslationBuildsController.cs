...
) : TranslationControllerBase(authService, urlService)
{
    ...
    /// Serval will wait until a build is completed after that date and time.
    /// </summary>
    ...
    [ProducesResponseType(typeof(void), StatusCodes.Status503ServiceUnavailable)]
    public async Task<ActionResult<TranslationBuildDto>> GetNextCompletedBuildAsync(
        [FromQuery(Name = "completed-after")] DateTime completedAfter,
        CancellationToken cancellationToken
    )
    {
        (_, EntityChange<Build> change) = await TaskEx.Timeout(
            ct => buildService.GetNextCompletedBuildAsync(Owner, completedAfter, ct),

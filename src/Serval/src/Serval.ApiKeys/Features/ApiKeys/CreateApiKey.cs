    ...
    : IRequestHandler<CreateApiKey, CreateApiKeyResponse>
{
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(config.ClientId))
            throw new InvalidOperationException("The client id must not be empty.");
...
public partial class ApiKeysController
{
    ...
    ///
    /// API key management is intended for administrative use only. A key can be created for any client with any

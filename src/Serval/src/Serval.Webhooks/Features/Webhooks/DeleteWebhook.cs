...
public class DeleteWebhookHandler(IDataAccessContext dataAccessContext, IRepository<Webhook> webhooks)
    : IRequestHandler<DeleteWebhook>
{
    public async Task HandleAsync(DeleteWebhook request, CancellationToken cancellationToken = default)
    {
        await dataAccessContext.WithTransactionAsync(
            async (ct) =>
            {
                await webhooks.CheckOwnerAsync(request.WebhookId, request.Owner, ct);

                Webhook? webhook = await webhooks.DeleteAsync(request.WebhookId, ct);
                if (webhook is null)

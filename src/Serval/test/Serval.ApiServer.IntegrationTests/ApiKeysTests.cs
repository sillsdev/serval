...
public class ApiKeysTests
{
        ...
        switch (expectedStatusCode)
        {
            case 201:
                ...
                Assert.Multiple(() =>
                {
                    ...
                    Assert.That(result.Owner, Is.EqualTo("client1"));
                });
    ...
    [Test]
    public async Task GetAllApiKeysAsync_FilterByOwner()
    {
        ApiKeysClient client = _env.CreateClient(null);
        await client.CreateAsync(
            new ApiKeyConfig
        ...
        IList<ApiKey> client2Keys = await client.GetAllAsync(owner: "client2");
    ...
    public async Task DeleteApiKeyByIdAsync(IEnumerable<string>? scope, int expectedStatusCode)
    {
        ...
        switch (expectedStatusCode)
        {
            case 200:
            {
                ...
                ServalApiException? ex = Assert.ThrowsAsync<ServalApiException>(async () =>
                {
                    await client.GetAsync(apiKey.Id);
                });

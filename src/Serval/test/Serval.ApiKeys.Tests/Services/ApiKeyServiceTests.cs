...
public class ApiKeyServiceTests
{
    ...
    public async Task CreateAsync_DoesNotStorePlaintextKey()
    {
        ...
        (ApiKey apiKey, string key) = await env.Service.CreateAsync(ClientId, "key1", ["read:files"], expiresAt: null);
    ...
    public async Task ValidateAsync_ValidKey()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(validated.ClientId, Is.EqualTo(ClientId));
    ...
    {
        var env = new TestEnvironment();
        (ApiKey _, string key) = await env.Service.CreateAsync(ClientId, "key1", ["read:files"], expiresAt: null);
        char lastChar = key[^1];
    ...
    [Test]
    public async Task ValidateAsync_RevokedKey()
    {
        var env = new TestEnvironment();
        (ApiKey apiKey, string key) = await env.Service.CreateAsync(ClientId, "key1", ["read:files"], expiresAt: null);
        await env.ApiKeys.UpdateAsync(k => k.Id == apiKey.Id, u => u.Set(k => k.RevokedAt, DateTime.UtcNow));

...
public class TranslationBuildJobService(
    IEnumerable<IBuildJobRunner<TranslationEngine>> runners,
...
) : BuildJobService<TranslationEngine>(runners, engines)
{
    ...
    )
    {
        ...
        if (engine?.CurrentBuild is not null)
        {
            ...
            IBuildJobRunner<TranslationEngine> runner = Runners[engine.CurrentBuild.BuildJobRunner];

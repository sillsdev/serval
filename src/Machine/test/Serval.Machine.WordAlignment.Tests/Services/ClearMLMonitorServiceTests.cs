...
public class ClearMLMonitorServiceTests
{
    ...
    // Service under test
    private ClearMLMonitorService<WordAlignmentEngine> _service;

    // Dependencies
    private IClearMLService _clearMLService;
    private ISharedFileService _sharedFileService;
    private ILogger<ClearMLMonitorService<WordAlignmentEngine>> _logger;
    ...
    public void Setup()
    {
        ...
        _service = new ClearMLMonitorService<WordAlignmentEngine>(
            _serviceProvider,
            _clearMLService,
            _sharedFileService,
            _clearMLOptions,
    ...
    private void SetupMocks()
    {
        ...
        _logger = Substitute.For<ILogger<ClearMLMonitorService<WordAlignmentEngine>>>();
    ...
    private static WordAlignmentEngine CreateTestEngine(
    ...
    {
        return new WordAlignmentEngine
        {
            EngineId = engineId,
            Type = EngineType.Nmt,
    ...
    {
        WordAlignmentEngine engine = CreateTestEngine();
    ...
    {
        WordAlignmentEngine engine = CreateTestEngine(jobState: BuildJobState.Active);
        SetupBuildingEngines(engine);
        ...
        {
            [ClearMLMonitorService<WordAlignmentEngine>.UserProperties] = new Dictionary<string, ClearMLParamsItem>
            {
    ...
    public async Task MonitorClearMLTasksPerDomain_CompletedStatus_ProperlyHandlesCompletion()
    {
        ...
        WordAlignmentEngine engine = CreateTestEngine(jobState: BuildJobState.Active, stage: BuildStage.Train);
        SetupBuildingEngines(engine);
        ...
        >
        {
            [ClearMLMonitorService<WordAlignmentEngine>.SummaryMetric] = new Dictionary<string, ClearMLMetricsEvent>
            {
                [ClearMLMonitorService<WordAlignmentEngine>.TrainCorpusSizeVariant] = new ClearMLMetricsEvent
                {
                    Value = ExpectedCorpusSize,
                },
                [ClearMLMonitorService<WordAlignmentEngine>.ConfidenceVariant] = new ClearMLMetricsEvent
                {
        ...
        await _wordAlignmentBuildJobService
            .Received(1)
            .StartBuildJobAsync(
                BuildJobRunnerType.Local,
                engine.Type,
    ...
    private void SetupBuildingEngines(WordAlignmentEngine engine)
    {
        _wordAlignmentBuildJobService
    ...
    private async Task VerifyCancellationFlow(WordAlignmentEngine engine)
    {
        await _platformService
            .Received(1)
    ...
    private async Task VerifyFailureHandling(WordAlignmentEngine engine, string errorReason, string errorMessage)
    {
        await _platformService
            .Received(1)
            .BuildFaultedAsync(
